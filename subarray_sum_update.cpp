...
int main(){
    ...
    cout << endl;
    // yay done
    cout << "Please enter a and b, the start and end indexes for the subarray, with a being less than b: ";
    cin >> a >> b;
    cout << "a and b are, respectively, " << a << " and " << b << endl;
    if (a <= n && b <= n && a > 0 && b >= 0 && a < b){
        int a_b_sum = tree[b] - tree[a - 1];
    ...
    if (c >= a && c <= b){
        int x = d - tree[c];
        add(c, x, tree);
        int new_a_b_sum = tree[b] - tree[a - 1];
